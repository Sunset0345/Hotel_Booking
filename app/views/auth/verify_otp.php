...
<head>
<meta charset="utf-8">
<title>Verify OTP - Blue Lagoon Hotel</title>
<style>
    ...
    input[type=text]{ width:100%; padding:0.6rem; margin-top:0.25rem; border-radius:6px; border:1px solid #e6e6e6; }
...
<div class="card">
    ...
    <form method="post" action="<?= site_url('auth/register') ?>">
        <label for="otp">Enter the OTP sent to your email<?= isset($email) ? ' ('.$email.')' : '' ?>:</label>
        <input id="otp" type="text" name="otp" required maxlength="6" />
        <button type="submit">Verify &amp; Create Account</button>
    </form>
    <form method="post" action="<?= site_url('auth/register') ?>" style="margin-top:1rem;">
        <input type="hidden" name="resend_otp" value="1" />
        <button type="submit" style="background:#036; color:#fff; padding:0.5rem 1rem; border-radius:6px; border:0; font-weight:600;">Resend OTP</button>
    </form>
    <div class="small">OTP is valid for 5 minutes.<br>
    <?php if(isset($resent) && $resent) echo '<span style="color:green">A new OTP has been sent to your email.</span>'; ?>
    </div>
</div>
</body>
</html>
